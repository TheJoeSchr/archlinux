...
# buildah run $BASE /bin/bash -c "bash < <(curl -s https://raw.githubusercontent.com/babashka/babashka/master/install)"

# SET WORKINGDIR /TMP
buildah config --workingdir /tmp "$BASE"
# COPY SCRIPTS /TMP
buildah copy "$BASE" "./*.sh" .
buildah copy "$BASE" "./*.csv" .

# RUN KEY INIT AND INSTALL BASICS
# buildah run "$BASE" /bin/bash "pwd"
# buildah run "$BASE" /bin/bash "ls -alkh"
buildah run "$BASE" /bin/bash "./init-pacman-keys.sh"
buildah run "$BASE" /bin/bash "./install-buildtools.sh"
